...
缺失编辑目标、视觉要求不明确或无法安全构造自动路径时，代理会先追问；仅在请求仍不明确且需要批准一份完整建议时才使用 `CONFIRM_IMAGE_ASSET`。`image_asset` 仅调用本机 Codex CLI 的内置 `image_gen`，不会静默使用 API fallback；调用可能消耗 Codex 额度。成功结果列出最终提示词、批次状态和逐输出发布状态，需视觉检查时再使用现有 `read` 能力。
