...
function makeDeps(
  ...
  return {
    ...
    subagentPolicy: {
      ...
      writePolicySnapshot: () => "/tmp/s.json",
...
describe("handleSessionStart", () => {
  it("sets debug enabled when log.debug is true", () => {
    ...
    createLifecycleHandler(deps).handleSessionStart(
  ...
  it("restores session rules on reload", () => {
    const sessionManager = {
      getEntries: () => [
        {
          type: "custom",
          customType: "my-permission:session-rule",
          data: { surface: "tools", pattern: "bash", action: "allow" },
        },
        {
          type: "custom",
          customType: "my-permission:session-rule",
          data: { surface: "paths", pattern: "*.env", action: "deny" },
        },
      ],
    };
    const deps = makeDeps();
  ...
  it("handles reload with no matching custom entries", () => {
    const sm = {
      getEntries: () => [{ type: "custom", customType: "other", data: {} }],
    };
    const deps = makeDeps();
    expect(() =>
      createLifecycleHandler(deps).handleSessionStart(
...
describe("handleSessionShutdown", () => {
  it("flushes logs", () => {
    ...
    createLifecycleHandler(deps).handleSessionShutdown(
      { type: "session_shutdown", reason: "quit" } as any,
      {} as any,
    );
