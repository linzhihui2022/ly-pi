...
import { detectInlineScript, type InlineScriptDetection } from "./detector";

const ESCALATION_THRESHOLD = 3;
const PREVIEW_LIMIT = 500;

export function buildReason(detection: InlineScriptDetection): string {
  return (
...
function buildConfirmMessage(
  detection: InlineScriptDetection,
  blockedCount: number,
...
export default function myScriptGuard(pi: ExtensionAPI): void {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const detection = detectInlineScript(event.input.command);
