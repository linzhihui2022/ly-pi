...
The Codex adapter uses argv-only `codex exec --ephemeral --sandbox workspace-write --json`, attaches Image Target before optional Reference Image, and asks built-in `image_gen` to copy its artifact into the prescribed staging path. It uses macOS `sips -g format` to accept only PNG inspection results. Process stderr is used only to classify transient failures and is never returned. Fake-runner/decoder tests cover ordered success, retry, second failure, invalid output, default no-overwrite, overwrite recovery, cancellation, command construction, and safe error mapping; no test invokes Codex.
