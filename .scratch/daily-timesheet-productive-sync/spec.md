...
- Every confirmed selection is revalidated by querying services with the current person, time tracking enabled, and the exact service name, reading each service record's ID, with up to 200 records per page. Using the returned `query_id` and `next_offset`, the skill follows only this exact-name query until it finds the original booking service ID or exhausts results. A same-named different ID is never substituted. A missing `query_id`, repeated or non-advancing offset, query failure, missing service ID, or malformed data blocks the batch; only a complete valid response without the original eligible ID skips the date as unavailable.
...
- Before final confirmation, the skill queries existing time entries for the current person, that candidate's per-date selected service, and same calendar day. Every page must be read through the initial `query_id` and returned `next_offset`; an initial or continuation query failure, missing `query_id`, repeated or non-advancing offset, missing/non-array `items`, or incomplete existing-entry data is a batch-blocking `Blocked`, never an empty result. An LLM compares every candidate note with those notes as `Same`/`Different`/`Uncertain`. The ticket-or-label is a weak identity signal: a matching label supports `Same`, while a mismatched or missing label does not alone rule it out.
