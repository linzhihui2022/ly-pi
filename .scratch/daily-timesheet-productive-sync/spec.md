...
- For every date with an allocation, the skill queries `bookings` for the current person with `booking_type=service`, `is_canceled=false`, `with_draft=true`, and `after`/`before` covering that date. The initial page requests at most 200 bookings with the booking schedule, stable service ID (retained internally only), service name, and optional task title; date-spanning bookings are included.
...
- Every confirmed selection is revalidated by querying services with the current person, time tracking enabled, and the exact service name, reading each service record's ID, with up to 200 records per page. Every initial response, including a terminal page, must carry a nonempty `query_id`; only an absent `next_offset` ends pagination, and a present `next_offset` must be a valid positive integer that strictly advances from the prior request. The skill follows only the original query ID until it finds the original booking service ID or exhausts results. A same-named different ID is never substituted. A missing/empty `query_id`, invalid/repeated/non-advancing `next_offset`, query failure, missing service ID, or malformed data blocks the batch; only a complete valid response without the original eligible ID skips the date as unavailable.
