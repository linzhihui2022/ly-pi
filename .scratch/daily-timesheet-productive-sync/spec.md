...
**本轮流程优化授权：** 用户随后明确批准在保持 booking、service 重验、既有工时重复预检、Review 和最终确认等安全边界不变的前提下，加入安全提速：无分配或全量 `Not scheduled` 时短路 Productive 后续阶段；独立只读调用可并行；所有日期的 service 确认合并为一次交互；同一运行内按 distinct service ID 去重重验；既有 `time_entries` 按 distinct service 的日期窗口查询后按精确日期分组；Review 决策合并为一次交互。本轮只修改源技能、规格和票据，不启用跳过重复检查的强制新建模式，不执行 Productive 写入、不部署。
...
- For every date with an allocation, the skill queries `bookings` for the current person with `booking_type=service`, `is_canceled=false`, `with_draft=true`, and `after`/`before` covering that date. Independent date queries may run in parallel. The initial page requests at most 200 bookings with the booking schedule, stable service ID (retained internally only), service name, and optional task title; date-spanning bookings are included.
- The skill consumes every booking page before deduplication or selection. Every initial response, including a terminal page, must carry a nonempty `query_id`; only an absent `next_offset` ends pagination. A present `next_offset` must be a valid positive integer that strictly advances from the prior request. A missing/empty `query_id`, invalid/repeated/non-advancing `next_offset`, continuation failure, missing/non-array `items`, or malformed booking/service data is a batch-blocking `Blocked` result, never an empty Scheduled on result.
- The skill deduplicates same-day bookings by service ID while retaining every underlying booking. A candidate displays the service name and, ordered by `started_on`, one `<optional task> — <booking-method summary>` segment for each booking; it never picks one task or aggregates mixed booking methods. IDs and Deal names remain hidden. One candidate requires confirmation; multiple candidates require an explicit selection. User-visible collisions across different service IDs are not selectable.
...
- Every distinct confirmed service ID is revalidated once per run by querying services with the current person, time tracking enabled, and the exact service name, reading each service record's ID, with up to 200 records per page; different service IDs may be revalidated in parallel. Every initial response, including a terminal page, must carry a nonempty `query_id`; only an absent `next_offset` ends pagination, and a present `next_offset` must be a valid positive integer that strictly advances from the prior request. The skill follows only the original query ID until it finds the original booking service ID or exhausts results, then binds that result to every selected date using the same service ID. A same-named different ID is never substituted. A missing/empty `query_id`, invalid/repeated/non-advancing `next_offset`, query failure, missing service ID, or malformed data blocks the batch; only a complete valid response without the original eligible ID skips the date as unavailable.
...
- Before final confirmation, the skill groups allocated dates by distinct selected service ID and queries existing time entries for the current person, each service, and a date range from that service's earliest to latest allocated date. It partitions returned records by exact `date + service` and ignores dates in the range without an allocation. Every response must contain an `items` array. A successful initial response with `items: []` and no `next_offset` is a complete empty result even when `query_id` is missing or empty; all candidates for that date and service continue as `Create`. Except for that terminal empty initial response, every initial response (including a non-empty terminal page) must carry a nonempty `query_id`; only an absent `next_offset` ends pagination, and a present `next_offset` must be a valid positive integer that strictly advances from the prior request. An initial or continuation query failure, missing/non-array `items`, a missing/empty initial `query_id` on a non-empty initial page or on any initial page with `next_offset`, invalid/repeated/non-advancing `next_offset`, or incomplete existing-entry data is a batch-blocking `Blocked`, never an empty result. An LLM compares every candidate note with those notes as `Same`/`Different`/`Uncertain`. The ticket-or-label is a weak identity signal: a matching label supports `Same`, while a mismatched or missing label does not alone rule it out.
...
- The final Daily Timesheet output retains its existing per-day summary and blind-spot reminders, then appends a concise synchronization receipt with selected Scheduled on services, `Not scheduled`, created, skipped, conflicted, reviewed, blocked, and failed entries. A `Blocked` receipt always names its reason and its known date or operation; it includes service, ticket, minutes, and note only when those fields already exist, never as guessed placeholders.

## Testing Decisions

- Test the workflow at the Daily Timesheet skill seam through a worked example rather than internal Productive implementation details. The example must begin with synthetic GitHub allocations and per-date Scheduled on bookings, include one service ID repeated across booking pages, render every booking segment for that one deduplicated candidate in `started_on` order without aggregating mixed booking methods, show a later booking page changing the deduplicated candidate set before selection, then show single-candidate confirmation and multiple-candidate choice for different per-date services, ID-preserving eligibility revalidation, preview, explicit final confirmation, and resulting draft entries bound to each date's selected service.
- The worked example must demonstrate that zero bookings, cancelled choices, unavailable selected services, and indistinguishable same-label services skip only their date; concrete one-page initial responses with a missing/empty `query_id` block booking lookup and revalidation, while an existing-entry initial response with `items: []`, no `next_offset`, and missing/empty `query_id` is a valid complete empty result that classifies candidates as `Create`; missing/empty `query_id` on a non-empty existing-entry page or on an existing-entry page with `next_offset`, and continuation responses with non-positive, non-integer, repeated, or non-advancing `next_offset`, block existing-entry lookup. It must prove there is no historical-entry or keyword/phrase fallback. Booking lookup and exact-name revalidation both consume all pages; only exact-name revalidation may paginate to locate the original booking service ID, and no broad service catalog is paginated.
- The example must prove a pre-candidate `Blocked` receipt with only a known date/operation, an existing-entry `Blocked` receipt that retains every known field including its note, and a later summary `Blocked` receipt that retains known service, ticket, and minutes while omitting its unavailable note; all prevent final confirmation and every create.
...
- Validation must concretely prove that a summary or comparison LLM timeout, unavailability, empty or malformed response, invalid summary, or missing comparison reason; an initial or continuation Scheduled on query/data failure; or an initial or continuation existing-time-entry query/pagination/data failure is `Blocked`, prints a blocked receipt, prevents final confirmation, and makes no create call.
- The optimization example must prove that no allocated date skips the remaining Productive phase, all `Not scheduled` dates skip summary/duplicate preflight/final confirmation, independent reads run in parallel, all service selections are confirmed in one interaction, a repeated service ID is revalidated once per run, and existing entries are fetched by per-service date windows then partitioned by exact date.
