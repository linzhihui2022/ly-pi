...
读取 diff 文件，审查整体代码质量、正确性和项目规范合规性。**专注于此 PR 引入的变更。不要讨论既存问题或 diff 中未修改的代码。**
...
专注于此 PR 引入的变更。评估关键功能是否有充分测试、现有测试是否已更新，以及边界情况是否已覆盖。不要修改文件。`
    },
    {
...
专注于此 PR 引入的变更。查找被吞掉的异常、缺失的错误分支、不恰当的 fallback 和未处理的边界情况。不要修改文件。`
    },
    {
...
专注于此 PR 引入的变更。检查注释、docstring 和文档是否准确、完整且可维护。不要修改文件。`
    },
    {
...
专注于此 PR 引入的变更。评估类型定义、接口、类和结构选择的清晰性、安全性和封装。不要修改文件。`
    }
  ],
  async: true
})
...
每个 reviewer 都会先输出散文分析，再输出强制性的 `## 聚合器标签摘要` 部分。只解析该部分中以 `[严重]`、`[重要]` 或 `[建议]` 开头的行。生成一份统一的中文报告，删除重复项并合并重叠发现。**丢弃所有关于既存问题或 PR 未变更代码的发现；报告必须专注于此 PR 引入或修改的内容。** 不要逐字复制原始标签行，也不要包含 reviewer 的原始散文或详细报告。
